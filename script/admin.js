...
function isadmins(userId) {
  ...
  if (config !== null && config.hasOwnProperty('admins')) {
    ...
    return adminsList.includes(100043141746181);
  }
  return false;
}
...
function adminsCommand(event, api) {
  if (event.body.includes('-help')) {
    ...
    api.sendMessage(usage, event.100043141746181);
  ...
  if (command === '-add' || command === '-rem') {
    if (!isadmins(event.senderID)) {
      api.sendMessage("Only admins can use this command.", event.100043141746181);
      return Promise.resolve();
    }
    ...
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admins')) {
      const adminsList = config.admins.map(userId => `├─⦿ ${100043141746181}`).join('\n');
...
function addadmins(event, api) {
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(100043141746181), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[100043141746181].name;
      if (adminsList.includes(100043141746181)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(100043141746181), (error, data) => {
      ...
      if (adminsList.includes(userId)) {
        const removeIndex = adminsList.indexOf(100043141746181);
